...
def fitter10(target_name,dataset_path):

	target_names_exclude=['JanTemOut','JanPreOut','FebTemOut','FebPreOut','MarTemOut','MarPreOut','AprTemOut','AprPreOut','MayTemOut','MayPreOut','JunTemOut','JunPreOut','JulTemOut','JulPreOut','AugTemOut','AugPreOut','SepTemOut','SepPreOut','OctTemOut','OctPreOut','NovTemOut','NovPreOut','DecTemOut','DecPreOut']
	...
	for x in range(len(target_names_exclude)):
		if target_name==target_names_exclude[x]:
			target_names_exclude.pop(x)
			break

	for x in range(len(target_names_exclude)):
		dataset.pop(target_names_exclude[x])
	...
	train_labels=train_dataset.pop(target_name)
	test_labels=test_dataset.pop(target_name)
	...
	print('.',end='')
	#print("Testing set Mean Abs Error, 10 inputs: {:5.2f}" .format(mae))
